...
void Node::init_routing_table()
{
	for(auto f : adapt_functions)
	{
		if (f->get_from() == 'a')
		{
			final_routing_table[0]->add_route(id, string(1,f->get_from()), 0, id, f, 'a', 0);
		}else
		{
			final_routing_table[1]->add_route(id, string(1,f->get_from()), 0, id, f, 'b', 0);
		}
	}
}


RoutingTable* Node::get_routing_table_a()
{
	return final_routing_table[0];
}

RoutingTable* Node::get_routing_table_b()
{
