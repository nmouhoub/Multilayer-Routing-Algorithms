...
void TopologyParser::configure_threads(Node* src, Node* dest, Link* link)
{
    src->add_link_out(link);
    dest->add_link_in(link);
    src->add_neighbor_queue(link->get_dest(), dest->get_queue());
    dest->add_neighbor_queue(link->get_src(), src->get_queue());
}
...
{   
    ...
    else
    {
        ...
        Network *network = new Network(generate_valid_protocols(nb_protocols), h_max);
...
void TopologyParser::generate_nodes_from_file(ifstream& topo_file, Network* network, int nb_nodes)
{
    ...
    for (int i = 0 ; i < nb_nodes ; i++)
    {
        ...
        nodes.insert(new Node(stoi(line), network->get_max_stack()));
    }
    topo_file.ignore(19,'\n');
...
void TopologyParser::generate_links_from_file(ifstream& topo_file, Network* network, int metric_cv, int metric_ec)
{
    ...
    while ( getline(topo_file,line) )
    {
        if(is_new_link(line)) 
        {
            ...
            configure_threads(network->get_node(src), network->get_node(dest), link); 
            map_cost = MapCost();
        }
        else 
        {
