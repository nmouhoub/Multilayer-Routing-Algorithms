...
bool RoutingTable::add_route(int dest, string protocol_stack, int cost, int next_hop, AdaptationFunction *adapt_func, char protocol_in, int h_max, int i)
{
	Index id = make_pair(dest,protocol_stack); 
	unordered_map<Index,Row>::iterator it = table.find(id); 
	if (it == table.end())
	{
		Row row = make_pair(make_pair(make_pair(make_pair(make_pair(cost, next_hop), adapt_func),protocol_in),h_max),i);
	...
	{	
		if ((it->second.first.first.first.first.first) > cost) 
		{
			...
			it->second.first.first.second = protocol_in;
